...
The codebase now contains the foundational Entities, DTOs with validation, and Exception Handlers to help us hit the "Exceeds Expectations" tier on the prototype grading rubric.
...
* **`controller/dto/`**: Contains the Data Transfer Objects (`ProjectDto`, `JobApplicationDto`, etc.). These include `javax.validation` tags (like `@NotBlank` and `@NotNull`) to validate frontend inputs before they hit the database.
* **`controller/exceptions/`**: Contains `ResourceNotFoundException` and a `GlobalExceptionHandler` to catch bad requests and prevent server crashes.
...
Linh, since you are leading the database management, your goal is to connect our entities to the H2 database and populate it with test data.
...
- [ ] **Initialize `data.sql`:** Open `src/main/resources/data.sql` and write the `INSERT INTO` statements to mock up our database. **Important:** The project specification requires us to have at least 50 total tuples (rows) across all our tables.
- [ ] **Test H2 Console:** Boot up the app and check `http://localhost:8085/h2-console` to ensure the tables are generating correctly.

## 🛠 Next Steps: Peter (System Architecture)
Peter, as my fellow System Architecture lead, we need to finish building out the REST API endpoints so the frontend team (Daisy, Anna, Fiona) can connect their UI.
...
- [ ] **Test with Insomnia:** Use the Insomnia REST client from Lab 2 to send `GET`, `POST`, `PUT`, and `DELETE` requests to ensure our DTO validations and Exception Handlers are working correctly.
