...
* **`controller/exceptions/`**: Contains `ResourceNotFoundException` and a `GlobalExceptionHandler` to catch bad requests and prevent server crashes[cite: 750].
...
Peter, as my fellow System Architecture lead [cite: 729][cite_start], we need to finish building out the REST API endpoints so the frontend team (Daisy, Anna, Fiona) can connect their UI.
